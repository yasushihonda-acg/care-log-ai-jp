...
## 9. JSONB vs Firestore: データモデル移行の影響

### 9.1 比較表

| 特性 | Vercel Postgres (JSONB) | Firestore (Document DB) |
|------|------------------------|------------------------|
| データ構造 | JSON in SQL列 | ネイティブDocument |
| クエリ | SQL + JSONBオペレータ | フィールドベースクエリ |
| インデックス | GINインデックス (手動) | **自動インデックス** |
| スキーマ | スキーマレス | スキーマレス |
| 部分更新 | `jsonb_set()` | `FieldValue.update()` |
| リアルタイム | 不可 | **onSnapshot()** |
| オフライン | 不可 | **クライアントSDKで対応** |

### 9.2 本プロジェクトへの影響

**維持される機能:**
- 柔軟なスキーマ (details MAP型)
- フィールド検索 (`details.fluid_type`)
- 部分更新
- ソート・フィルタ
...
**JSONBの方が有利だったケース:**
- 複雑な集計 (SUM, AVG on JSON fields)
- 複雑なJOIN

### 9.3 結論

本プロジェクトの単純なCRUD + AI解析ユースケースでは、**Firestoreで十分対応可能**です。
将来的に複雑な集計が必要になった場合は、BigQueryへのデータエクスポートを検討します。
