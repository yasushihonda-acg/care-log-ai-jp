...
### Added - CSVエクスポート機能
- **履歴タブにCSVダウンロードボタン追加**: フィルタ適用後のデータをCSVでエクスポート可能
- **Excel互換フォーマット**: UTF-8 with BOM で日本語文字化けを防止
- **動的カラム生成**: fieldSettingsに基づいてヘッダーラベルを自動生成
- **ファイル名自動生成**: `care_records_YYYYMMDD_HHmmss.csv` 形式

### Documentation
- **CSV_EXPORT_SPEC.md**: CSVエクスポート機能の設計書を追加
